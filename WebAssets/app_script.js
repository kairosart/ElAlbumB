...
function mejorarFotoBoda(e) {
  try {
    ...
    // 🛠️ MODIFICADO: Ahora el estado pasa a escribirse en la columna 11 (Columna K)[cite: 3]
    sheet.getRange(fila, 11).setValue("Pendiente");
    ...
    console.log(`✉️ [PACK EXPRESS] Instrucciones enviadas a ${emailCliente}. Fila: ${fila}. Estado 'Pendiente' en Columna K.`);
...
function revisarGmailYProcesarPedidos() {
  ...
  for (let h = 0; h < hilos.length; h++) {
    ...
    for (let i = 1; i < datos.length; i++) {
      ...
      // 🛠️ MODIFICADO: El array de datos busca el estado en la columna K (Índice 10)[cite: 3]
      const estado = datos[i][10] ? datos[i][10].toString().trim() : ""; 
      ...
      if (estado === "Pendiente" && nombreClienteForm !== "") {
        ...
        if (cuerpoEmailLimpio.includes(nombreClienteLimpio)) {
          ...
          hilos[h].markRead();
          // 🛠️ MODIFICADO: Se actualiza a la columna 11 (Columna K)[cite: 3]
          sheet.getRange(i + 1, 11).setValue("Procesando"); 
...
function ejecutarReveladoGemini(sheet, filaIndex, emailCliente, estiloElegido, fileIdsRaw) {
  const nombrePestana = sheet.getName().trim().toUpperCase();
  // 🛠️ MODIFICADO: Si es PACK EXPRESS, ahora la columna destino es la 11 (K)[cite: 3]
  const colEstado = (nombrePestana === "PRUEBA GRATIS") ? 6 : 11;
 ...
 * Configura la URL, el nombre de la carpeta y el tipo de evento que deseas procesar.
 */
function ejecutarMuestrarioManual() {
  // ⚠️ CONFIGURA ESTOS DATOS ANTES DE EJECUTAR
  const urlFotoOriginal = "https://drive.google.com/file/d/1nrmVxHfUhk9mSbSaaRB-PJRh3e_BnWwe/view?usp=drive_link"; 
  const identificadorCarpeta = "Muestrario_Premium_Clientes"; // Nombre base de la carpeta
  
  /**
   * 🔀 TIPO DE EVENTO REQUERIDO:
   * Opciones: "boda" | "comunion" | "bautizo" | "todos"
   * (El script buscará estas palabras clave dentro del nombre de tus estilos en ESTILOS_PROMPT)
   */
...
function generarTodosLosEstilos(urlFoto, nombreCarpeta, tipoEvento) {
  try {
    ...
    // 1. Validar y ajustar el nombre final de la carpeta según el filtro
    ...
    // 2. Preparar la carpeta de destino en Google Drive
    let carpetaRaiz = DriveApp.getFolderById(CARPETA_RAIZ_ID.trim());
    ...
    // 4. Descargar y codificar la foto original
    const archivoOriginal = DriveApp.getFileById(fileId);
    const blob = archivoOriginal.getBlob();
    const base64Image = Utilities.base64Encode(blob.getBytes());
    const mimeType = blob.getContentType();
    ...
    // 5. Filtrar la lista de estilos dinámicamente según la opción elegida
    ...
    // 6. Bucle de procesado para invocar a Gemini por cada estilo
    for (let i = 0; i < listaEstilos.length; i++) {
      ...
      // Determinar dinámicamente el contexto para ajustar la orden inicial enviada a la IA
      ...
      // Combinación inteligente del prompt final neutro
      ...
      // Sistema Antibloqueo contra errores 429 y 503
      while (intentos < MAX_INTENTOS && !llamadaExitosa) {
      ...
      if (imagePart) {
        const imageData = imagePart.inlineData;
        // Reemplaza espacios por guiones bajos para que el nombre de archivo quede limpio
      ...
      // ⏸️ Pausa protectora de cuota para evitar bloqueos en Google App Script
...
function ejecutarMuestrarioComunionesManual() {
  ...
  const identificadorCarpeta = "Portfolio_Comuniones"; // Nombre para la carpeta
...
function generarTodosLosEstilosComuniones(urlFoto, nombreCarpeta) {
  try {
    // Validar que la URL no llegue vacía o undefined
    if (!urlFoto) {
    ...
    // Verificar que la constante global exista antes de usar .trim()
    ...
    // Verificar que la constante MODELO exista
    ...
    // 3. Extraer SOLO los estilos de comuniones (soporta comunion y comunión)
...
/**
 * Extrae de forma segura el ID de una URL de Google Drive protegiendo contra valores undefined.
 */
function extraerIdDeUrlDrive(url) {
  // Protección: Si no es un string válido, salir inmediatamente sin ejecutar .split()
