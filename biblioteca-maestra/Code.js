// @ts-nocheck
// =================================================================================
// CONFIGURACIÓN GLOBAL (Obtiene las variables de configuración dinámicas de la pestaña "CONFIGURACIÓN"
// =================================================================================

const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("CONFIGURACIÓN");
  if (!sheet) {
    throw new Error("No se encontró la pestaña 'CONFIGURACIÓN'. Asegúrate de que el nombre sea exacto.");
  }
...
function ejecutarReveladoGemini(sheet, filaIndex, emailCliente, estiloElegido, fileIdsRaw) {
  ...
  try {
    ...
    let fotosOk = 0;
    let fotosError = 0;
    let fotosBloqueadasSeguridad = 0;
    ...
    for (let i = 0; i < fileIds.length; i++) {
      ...
      try {
        ...
        // 1. Detección si la API da un error HTTP directo (ej. Código 400 Bad Request por filtros automáticos)
        if (json.error) {
          const errorDetail = JSON.stringify(json.error).toLowerCase();
          
          if (errorDetail.includes("safety") || errorDetail.includes("block") || errorDetail.includes("policy") || errorDetail.includes("violation")) {
            console.warn(`⚠️ Foto ${i+1} rechazada por políticas de seguridad en la petición.`);
            enviarMailRestriccion(emailCliente, archivoOriginal.getName());
            fotosBloqueadasSeguridad++;
          } else {
        ...
        // 2. Detección de bloqueo silencioso (La API devuelve 200 OK pero el "finishReason" es SAFETY)
        const primerCandidato = json.candidates && json.candidates[0];
        if (primerCandidato && primerCandidato.finishReason === "SAFETY") {
          console.warn(`⚠️ Foto ${i+1} bloqueada tras el análisis (finishReason: SAFETY).`);
          enviarMailRestriccion(emailCliente, archivoOriginal.getName());
          fotosBloqueadasSeguridad++;
          continue;
        }

        // 3. LEER CON TOTAL SEGURIDAD los "parts" evitando "Cannot read properties of undefined (reading 'parts')"
        const parts = (primerCandidato && primerCandidato.content && primerCandidato.content.parts) ? primerCandidato.content.parts : null;
        ...
        if (imagePart) {
          ...
          // Si el candidato terminó por motivos de seguridad pero no capturados por el filtro anterior
          if (primerCandidato && primerCandidato.finishReason && primerCandidato.finishReason !== "STOP") {
            console.warn(`⚠️ Foto ${i+1} no procesada. Razón de finalización: ${primerCandidato.finishReason}`);
            enviarMailRestriccion(emailCliente, archivoOriginal.getName());
            fotosBloqueadasSeguridad++;
          } else {
        ...
        // 4. Captura en try/catch por si el flujo falla internamente por un filtro inesperado
        const errorMsg = err.toString().toLowerCase();
        if (errorMsg.includes("safety") || errorMsg.includes("blocked") || errorMsg.includes("parts")) {
          console.warn(`⚠️ Excepción de seguridad/procesamiento controlada en foto ${i+1}.`);
          try {
            const archivoOriginal = DriveApp.getFileById(fileIds[i]);
            enviarMailRestriccion(emailCliente, archivoOriginal.getName());
          } catch(e) {
            enviarMailRestriccion(emailCliente, "Archivo de Imagen");
          }
          fotosBloqueadasSeguridad++;
        } else {
    ...
    // Lógica para determinar el resultado del proceso en la hoja de cálculo
    if (fotosOk > 0) {
      ...
      if (nombrePestana === "PACK EXPRESS") {
        ...
        const cuerpoEntrega = `
          ...
          ${fotosBloqueadasSeguridad > 0 ? `<p>⚠️ <i>Nota: Algunas de las imágenes enviadas no pudieron ser procesadas debido a filtros de privacidad automáticos. Te hemos enviado un correo detallando la situación.</i></p>` : ""}
      ...
      // Si no se pudo procesar ninguna foto y hubo bloqueos por privacidad, el estado lo refleja de forma más exacta
      if (fotosBloqueadasSeguridad > 0 && fotosError === 0) {
        sheet.getRange(filaIndex, colEstado).setValue("Bloqueo Privacidad");
      } else {
        sheet.getRange(filaIndex, colEstado).setValue("Error Gemini");
      }
      return null;
...
/**
 * Envía un correo electrónico explicativo cuando una imagen ha sido rechazada por filtros automáticos de seguridad.
 */
function enviarMailRestriccion(emailCliente, nombreArchivo) {
  try {
    const asunto = "⚠️ Nota sobre la imagen subida a El Álbum B";
    const cuerpoHtml = `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 8px;">
        <h2 style="color: #c9a054; text-align: center;">El Álbum B</h2>
        <p>¡Hola!</p>
        <p>Queríamos informarte de que la imagen llamada <b>"${nombreArchivo}"</b> que has intentado procesar no ha podido completarse.</p>
        
        <div style="background-color: #f9f9f9; border-left: 4px solid #c9a054; padding: 15px; margin: 20px 0;">
          <p style="margin: 0; font-weight: bold;">¿Por qué ha ocurrido esto?</p>
          <p style="margin: 5px 0 0 0; font-size: 0.9em; color: #666;">
            Los servidores de procesamiento de Google aplican algoritmos de privacidad automáticos sumamente rigurosos que escapan a nuestro control. En ocasiones, fotos de preparativos de boda (donde aparece lencería, batas, vestidos de encaje, transparencias o personas vistiéndose) son interpretadas erróneamente por estos filtros de seguridad global, impidiendo que la IA genere el revelado.
          </p>
        </div>
        
        <p><b>¿Qué te sugerimos hacer?</b></p>
        <ul>
          <li>Prueba a subir una fotografía con un encuadre alternativo o un plano más cerrado de los detalles (por ejemplo, el peinado, los zapatos o el ramo).</li>
          <li>Asegúrate de que la toma principal no confunda a los detectores automáticos de privacidad.</li>
        </ul>
        
        <p>Disculpa el inconveniente, ¡nos encantará procesar el resto de tus fotos para que queden perfectas!</p>
        <hr style="border: 0; border-top: 1px solid #eee; margin: 20px 0;">
        <p style="font-size: 0.8em; color: #999; text-align: center;">Mensaje automático de control de procesamiento - El Álbum B.</p>
      </div>
    `;

    GmailApp.sendEmail(emailCliente, asunto, "", { htmlBody: cuerpoHtml });
    console.log(`✉️ Correo de restricción de privacidad enviado a: ${emailCliente}`);
  } catch (errorMail) {
    console.error("🔴 Error intentando enviar el correo de restricción: " + errorMail.toString());
  }
}
